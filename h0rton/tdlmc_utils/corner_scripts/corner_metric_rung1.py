...
import glob
dirpath = os.getcwd()
folders = ['Tak', 'MartinMillon','freeform','Rathnakumar', 'Rathnakumar1']
subnames = []
...
for subname in subnames:
    if 'h0' in subname or 'txt' in subname:
        ...
        if len(submit) > 0:
            ...
            goodness = round(1/float(len(submit))*np.sum(((submit[:,0]-h0_rung1)/submit[:,1])**2),3)
            precision = round(1/float(len(submit))*np.sum(submit[:,1]/h0_rung1)*100, 3)
            accuracy = round(1/float(len(submit))*np.sum((submit[:,0]-h0_rung1)/h0_rung1)*100, 3)
